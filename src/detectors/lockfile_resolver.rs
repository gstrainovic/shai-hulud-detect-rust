...
impl LockfileResolver {
    ...
    fn load_pnpm_lockfile<P: AsRef<Path>>(dir: P) -> Result<Self> {
        ...
        for line in content.lines() {
            ...
            // Pattern 2: Packages section: "/debug@4.3.4:"
            if trimmed.starts_with('/') && trimmed.contains('@') && trimmed.ends_with(':') {
                let pkg_line = trimmed.trim_start_matches('/').trim_end_matches(':');
...
fn extract_npm_packages(deps: &HashMap<String, NpmPackage>, packages: &mut HashMap<String, String>) {
...
"#;
    ...
    fn test_extract_package_name() {
        assert_eq!(extract_package_name("node_modules/debug"), Some("debug".to_string()));
        assert_eq!(extract_package_name("node_modules/@scope/package"), Some("@scope/package".to_string()));
