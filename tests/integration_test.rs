...
fn test_normal_mode_100_percent_match() {
    ...
    assert!(stdout.contains("Medium Risk Issues: 58"), "MEDIUM should be 58");
    assert!(stdout.contains("Low Risk (informational): 9"), "LOW should be 9");
}

#[test]
fn test_infected_project_normal_mode() {
    let output = Command::new("cargo")
        .args(&["run", "--release", "--", "../shai-hulud-detect/test-cases/infected-project"])
        .output()
        .expect("Failed to run scanner");
    ...
    assert!(stdout.contains("Medium Risk Issues: 8"), "MEDIUM should be 8");
    assert!(stdout.contains("Low Risk (informational): 2"), "LOW should be 2");
}

#[test]
fn test_clean_project() {
    let output = Command::new("cargo")
        .args(&["run", "--release", "--", "../shai-hulud-detect/test-cases/clean-project"])
        .output()
        .expect("Failed to run scanner");
    ...
    assert!(stdout.contains("No indicators of Shai-Hulud compromise detected"), 
...
fn test_paranoid_mode_enhanced_security() {
    ...
    assert!(stdout.contains("High Risk Issues: 8"), "HIGH should be 8");
    assert!(stdout.contains("Medium Risk Issues: 19"), "MEDIUM should be 19 in paranoid");
    assert!(stdout.contains("Low Risk (informational): 2"), "LOW should be 2");
}
...
fn test_homoglyph_detection() {
    ...
    assert!(stdout.contains("Unicode/homoglyph") || stdout.contains("reаct"),
